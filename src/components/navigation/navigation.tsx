...
const Navigation: React.FC<Props> = ({ navVisible, setNavVisible }) => (
    <div className='navigation'>
        <button className={`nav-toggle ${navVisible ? 'nav-invisible' : ''}`} onClick={() => setNavVisible(true)}>
            <MenuIcon size={60} />
        </button>
        <nav className={`nav ${navVisible ? 'nav-visible' : ''}`} data-testid='nav'>
            <button className='close-button' onClick={() => setNavVisible(false)}>
                <CloseIcon size={30} />
            </button>
